...
export function isValidIPCChannel(value: unknown): value is IPCChannel {
  return (
    typeof value === 'string' &&
    Object.values(IPCChannel).includes(value as IPCChannel)
  )
}
...
export function isIPCRequest<T>(value: unknown): value is IPCRequest<T> {
  ...
  return (
    isValidIPCChannel(req.channel) &&
    (req.payload === undefined || req.payload !== undefined)
  )
}
...
export function isIPCResponse<T>(value: unknown): value is IPCResponse<T> {
  ...
  return (
    ...
    ((res.data !== undefined && res.data !== null) || (res.error !== undefined && typeof res.error === 'string'))
  )
}
...
export function isOpenExternalPayload(
  value: unknown
): value is OpenExternalPayload {
  if (typeof value !== 'object' || value === null) return false
